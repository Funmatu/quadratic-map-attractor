ご指摘の通りです。「特定の画像に依存する暗黙知」や「文脈を共有していないと理解できない表現」を完全に排除し、本プロジェクトが何を目的とし、どのような数理モデルに基づき、どのようなアーキテクチャで実装されるのかを、ゼロから完全に明示化（Explicitに記述）した体系的・網羅的なシステム設計・仕様書を提示します。

R&Dプロジェクトの公式な要件定義書および実装ロードマップとして機能するよう、極めて詳細な粒度で記述しています。
...
$$I = \int_{L}^{U} f(z) \, dz$$
ここで、$U$ は上端の積分値、$L$ は下端の積分値を示します。自己相似なフラクタル構造を仮定し、被積分関数 $f(z)$ が自分自身の状態 $I$ と変数 $z$ の積に比例する（$f(z) = I \cdot z$）と定義します。この定積分を解析的に評価します。
...
ここで、$n$ は離散的な時間ステップ（反復回数）、$k$ はシステム全体の結合強度を制御するスカラーパラメータです。本システムは、数百万のパーティクル（点）に微小に異なる摂動パラメータを与え、この漸化式を毎フレーム並列計算することで、ストレンジアトラクタを生成します。
...
* **仕様:** 数百万パーティクルの初期状態（$I_0, U_0, L_0$）と、それぞれの空間的摂動パラメータ（$C_I, C_U, C_L$）を乱数で生成します。WebGPUのメモリ制約に準拠するため、WASMの線形メモリ上に連続したフラット配列（SoA: Structure of Arrays）としてデータを展開し、JavaScript側にメモリアドレス（ポインタ）のみをエクスポートします。
...
* **仕様:** compute.wgsl に2.2で定義した複素連立漸化式を実装します。GPU上でパーティクルごとにスレッドを割り当て、$n \to n+1$ の状態更新を行います。計算が無限大に発散するのを防ぐため、状態ベクトルのノルム（絶対値）が閾値を超えた場合に原点にリセットする機構（ポアンカレ断面的な制御）を組み込みます。



* **Render Pipeline (位相幾何学的可視化):**
* **仕様:** render.wgsl において、6次元空間の情報を2Dスクリーンに射影します。具体的には、状態 $I$ の実部と虚部をキャンバスのXY座標に対応させます。同時に、状態 $U$ と $L$ の複素平面上での位相角（偏角）を計算し、その干渉パターンをRGB色相にマッピングすることで、力学系内部のトポロジー構造を光の色として視覚化します。
